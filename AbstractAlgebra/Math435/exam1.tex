...
\section*{Permutations, Cosets, and Direct Products}
	\subsection*{Groups of Permutations}
		...
		\1	\Definition \textbf{S3}
			\2	Symmetric group on $3$ letters
			\2	$|S_3| = 3! = 6$
			\2	Elements of the group
				\3	$\rho_0 = \begin{pmatrix}
												1 & 2 & 3 \\
												1 & 2 & 3 \\
											\end{pmatrix}$
				\3	$\rho_1 = \begin{pmatrix}
												1 & 2 & 3 \\
												2 & 3 & 1 \\
											\end{pmatrix}
											$
				\3	$\rho_2 = \begin{pmatrix}
												1 & 2 & 3 \\
												3 & 1 & 2 \\
											\end{pmatrix}$
				\3	$\mu_1 = \begin{pmatrix}
											1 & 2 & 3 \\
											1 & 3 & 2 \\
										\end{pmatrix}$
				\3	$\mu_2 = \begin{pmatrix}
											1 & 2 & 3 \\
											3 & 2 & 1 \\
										\end{pmatrix}$
				\3	$\mu_3 = \begin{pmatrix}
												1 & 2 & 3 \\
												2 & 1 & 3 \\
											\end{pmatrix}$
		\1	\Definition \textbf{Cyclic Notation}
			\2	$\mu = (1,3,6)$
			\2	$\mu = \begin{pmatrix}
										1 & 2 & 3 & 4 & 5 & 6 & 7 & 8 \\
										3 & 2 & 6 & 4 & 5 & 1 & 7 & 8 \\
									\end{pmatrix}$
		\1	\Definition \textbf{Disjoint Cycle}
			\2	Any integer is moved by at most one cycle when written as a product
		\1	\Theorem Every permutation $\sigma$ of a finite set is a product of disjoint cycles
	
	\subsection*{Cosets}
		\1	\Definition \textbf{Left Coset}
			\2	$H \le G$
			\2	$aH = \{ah | h \in H\}$ of $G$ 
			\2	Left coset of $H$ containing $a$
		\1	\Theorem \textbf{Lagrange's Theorem}
			\2	$H \le G$, $\abs{G} < \infty$
			\2	$\frac{\abs{G}}{\abs{H}} = n \in \mathbb{N}$
		\1	\Theorem Every group of prime order is cyclic
		\1	\Theorem $\frac{\abs{G}}{\abs{\langle g \rangle}} = n \in \mathbb{N}$
		\1	\Definition \textbf{Index}
			\2	Number of left cosets of $H$ in $G$
			\2	$[G~:~H]$ is the index of $H$ in $G$
			\2	$[G~:~H] = \abs{G}/\abs{H}$
		\1	\Theorem $K \le H \le G$ then $[G~:~K] = [G~:~H][H~:~K]$
